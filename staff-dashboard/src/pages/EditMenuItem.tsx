...
const EditMenuItem: React.FC = () => {
  ...
  const fetchMenuItem = async () => {
    try {
      ...
      // Convert dietaryInfo object to array
      const dietaryArray: string[] = [];
      if (item.dietaryInfo?.vegetarian) dietaryArray.push('Vegetarian');
      if (item.dietaryInfo?.vegan) dietaryArray.push('Vegan');
      if (item.dietaryInfo?.glutenFree) dietaryArray.push('Gluten-Free');
      if (item.dietaryInfo?.dairyFree) dietaryArray.push('Dairy-Free');
      if (item.dietaryInfo?.halal) dietaryArray.push('Halal');
      if (item.dietaryInfo?.kosher) dietaryArray.push('Kosher');

      // Extract allergen IDs
      const allergenIds = item.allergens?.map((a: any) => a._id) || [];
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-4">
              ...
              <div>
                ...
                <p className="text-sm text-gray-500 mt-0.5">Update the details of your menu item</p>
              </div>
            </div>
